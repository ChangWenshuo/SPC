...
for subj in SubjFiles:
    ...
    if not os.path.exists(opj(OutPathSub, SubNum + '_eogscore.png')):
    ...
    if not os.path.exists(opj(OutPathSub, SubNum + '_emgscore.png')):
        fig = ica.plot_scores(muscle_scores, exclude=muscle_ind)
        fig.savefig(opj(OutPathSub, SubNum + '_emgscore.png'))
        plt.close('all')
    ##### ICs to exclude
    exclude_list = list((set(elab_indices) & set(eog_ind)) | set(hlab_indices))
    exclude_list.sort()
    ...
    plt.close('all')
    del epochs_recon, ica, fig, exclude_list
